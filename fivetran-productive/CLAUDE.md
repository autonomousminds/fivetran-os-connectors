...
## IMPORTANT: Never run connector.py or deploy yourself

- NEVER run `python connector.py` or `connector.debug()` yourself. Always ask the user to run it and share the output. This avoids consuming API quota, hitting rate limits, or making unintended API calls against production data.
- NEVER run `fivetran deploy` yourself. Always provide the command and let the user execute it.
- NEVER run `git push` yourself. Always ask the user to push. Only commit locally when asked.
