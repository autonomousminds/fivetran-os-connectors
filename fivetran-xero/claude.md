...
2. KNOWLEDGE BASE
- Deep understanding of Fivetran Connector SDK (v2.0+ — no yield pattern)
...
- Xero API Documentation:
  ...
  - [Xero OpenAPI Specs (authoritative field definitions)](https://github.com/XeroAPI/Xero-OpenAPI)
  ...
  - [Built-in Fivetran Xero Connector (parity reference)](https://fivetran.com/docs/connectors/applications/xero)
...
1. SCHEMA DEFINITION
- Define table names, primary keys, and explicit PK column types (prevents empty-table type inference errors)
- Example:
```python
def schema(configuration: dict):
    return [
        {"table": "table_name", "primary_key": ["key"], "columns": {"key": "STRING"}}
    ]
```
