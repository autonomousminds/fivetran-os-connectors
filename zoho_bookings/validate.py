"""
Local validation runner for the Zoho Bookings connector.
...
print(f'  duckdb {db_path} -c "SELECT COUNT(*) FROM appointments;"')
print(f'  duckdb {db_path} -c "SELECT booking_id, customer_name, service_name, start_time, status FROM appointments LIMIT 10;"')
print(f'  duckdb {db_path} -c "SELECT s.name, st.name FROM services s JOIN service_staff_assignments ssa ON s.id = ssa.service_id JOIN staff st ON st.id = ssa.staff_id LIMIT 10;"')
con.close()
