"""
Local validation runner for the Zoho Projects connector.
...
delete` to buffer records in Python, then writes them to a local DuckDB
file via the native ARM64 `duckdb` package — no JVM, no Rosetta, no
crashes.

Same `update()` code path as production. If this script completes
cleanly, `fivetran deploy` should also work; the JVM crashes are a
local-tester artefact only.
...
REQUIRED = ("client_id", "client_secret", "refresh_token", "data_center")
...
if os.path.exists(_state_path):
    with open(_state_path) as f:
        state = json.load(f)
    print(f"Resuming from local state: keys={list(state.keys())[:10]}"
          f"{'...' if len(state) > 10 else ''}")
...
print(f'  duckdb {db_path} -c "SELECT COUNT(*) FROM portals;"')
print(f'  duckdb {db_path} -c "SELECT COUNT(*) FROM projects;"')
print(f'  duckdb {db_path} -c "SELECT COUNT(*) FROM tasks;"')
print(f'  duckdb {db_path} -c "SELECT COUNT(*) FROM bugs;"')
print(f'  duckdb {db_path} -c "SELECT COUNT(*) FROM time_logs;"')
print(f'  duckdb {db_path} -c "SELECT field_api_name, COUNT(*) FROM task_custom_fields GROUP BY 1;"')
con.close()
